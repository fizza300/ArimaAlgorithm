...
Original file is located at
    https://colab.research.google.com/drive/13yfJJb87sfcI_jEBDmUba-f_fCIfawWq
"""
...
plt.plot(pd.date_range(start=df['date_of_data'].max(), periods=30, freq='D'), forecast, label='Forecast', linestyle='--')
plt.xlabel('Date')
plt.ylabel('Percentage')
...
forecast_df = pd.DataFrame({
    'Date': pd.date_range(start=df['date_of_data'].max() + pd.Timedelta(days=1), periods=30, freq='D'),
    'Forecast': forecast
})
